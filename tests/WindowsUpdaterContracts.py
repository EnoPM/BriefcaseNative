...
class Contracts(unittest.TestCase):
 ...
 def test_paths_json_archive_and_release_identity(self):
  ...
  content=json.loads(release.read_text());content['assets']=[];write_json(release,content)
  self.assertEqual(self.run_native('select',release,'Example/Framework','1.0.0'),'null')
  write_json(release,self.release(archive))
 ...
 def test_mod_update_preserves_configuration(self):
  ...
  write_json(release_path,dict(release,assets=[]))
  waiting=json.loads(self.run_native('mods',self.root,release_path,archive,repository,'7'))
  self.assertEqual((waiting['current'],waiting['failed']),(1,0))
