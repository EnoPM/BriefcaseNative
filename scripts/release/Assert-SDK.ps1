...
foreach($record in $manifest.files){
 ...
 if($record.path -notmatch '^(include/Briefcase/.+\.(h|hpp)|third_party/include/nlohmann/.+\.hpp|Licenses/nlohmann-json\.txt|cmake/BriefcaseNativeSDKConfig(Version)?\.cmake)$' -or -not $seen.Add($path)){throw 'Unexpected SDK file'}
...
foreach($required in @('include/Briefcase/ModApi.h','include/Briefcase/Unreal.hpp','third_party/include/nlohmann/json.hpp','cmake/BriefcaseNativeSDKConfig.cmake')){
